...
public class BillingUtilities {
     ...
     */
    @Nullable
    public static boolean getSubscriptionForSku(String sku) {
        if(sku.equals(Constants.PREMIUM)){
            return LiveTvApplication.getUser().getSubscription().getMembership() == 1;
        } else if(Constants.PREMIUM_PLUS.equals(sku)){
            return LiveTvApplication.getUser().getSubscription().getMembership() == 2;
        }
        return false;
    }
    ...
    public static boolean serverHasSubscription(
            String sku) {
        return getSubscriptionForSku(sku);
    }
    ...
    public static boolean isBasicContent(@Nullable SubscriptionStatus subscription) {
        return subscription != null &&
                subscription.isEntitlementActive &&
                Constants.PREMIUM.equals(subscription.sku) &&
                !subscription.subAlreadyOwned;
    }
    ...
    public static boolean isPremiumContent(@Nullable SubscriptionStatus subscription) {
        return subscription != null &&
                subscription.isEntitlementActive &&
                Constants.PREMIUM_PLUS.equals(subscription.sku) &&
                !subscription.subAlreadyOwned;
    }
