...
/* package */ final class TrackSelectionHelper implements View.OnClickListener,
  ...
  private View buildView(Context context) {
    ...
    View view = inflater.inflate(R.layout.video_player_track_selector_dialog, null);
    ...
    disableView.setText(context.getResources().getString(R.string.selection_disabled));
    disableView.setBackground(context.getResources().getDrawable(R.drawable.dialog_btn_background));
    ...
    for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
      ...
      for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
        if (trackIndex == 0) {
          root.addView(inflater.inflate(R.layout.video_player_list_divider, root, false));
        }
        ...
        trackView.setBackground(context.getResources().getDrawable(R.drawable.dialog_btn_background));

        //SuperTV subtitle name
        String tagToAdd = buildTrackName(group.getFormat(trackIndex));
        if(!tagToAdd.equals(context.getString(R.string.tag_esp_srt))) {
          if (tagToAdd.toLowerCase().contains("spa") || tagToAdd.toLowerCase().contains("id:3")) {
            tagToAdd = context.getString(R.string.tag_esp);
          } else if (tagToAdd.toLowerCase().contains("eng")) {
            tagToAdd = context.getString(R.string.tag_eng);
          }
        }
        trackView.setText(tagToAdd);
